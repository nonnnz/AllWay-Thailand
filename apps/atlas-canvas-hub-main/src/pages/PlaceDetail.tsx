...
import {
  getPlace, getTrust, getFairPrice, getCulturalContext, getGraph, postReport, queryKeys,
} from '@/lib/api/client';
      ...
      <div className="container py-6 md:py-10">
        ...
        <div className="mt-4 grid grid-cols-1 gap-8 lg:grid-cols-[1.35fr_1fr]">
          ...
          <aside className="lg:sticky lg:top-24 lg:h-fit space-y-4">
            <PlacesMap pins={[{ id: p.id, lat: p.lat, lng: p.lng, label: p.name, sublabel: p.provinceName }]} height="320px" zoom={9} />
            <button className="w-full rounded-md bg-primary py-3 text-sm font-semibold text-primary-foreground hover:bg-primary-hover">
              {t('cta.saveItinerary')}
            </button>
            <button
