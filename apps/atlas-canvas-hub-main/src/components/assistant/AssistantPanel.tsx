...
function ResponseCard({
  ...
  return (
    <div className="max-w-[92%] rounded-2xl rounded-bl-sm border border-border bg-surface p-3 text-sm">
      {riskBadge && (
        <div
          className={cn(
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {response.suggestedActions.map((a) => (
            <button
              key={a}
              onClick={() => onAction(a)}
              className="rounded-full border border-border bg-background px-2.5 py-1 text-xs font-medium text-foreground transition-colors hover:bg-primary-soft hover:text-primary"
            >
